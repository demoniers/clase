...
socket.on("chat message", (msg) => {
  if (!msg.recipient) {
    ...
    const newMessage = document.createElement("div");
    newMessage.textContent = `${msg.username}: ${messageContent}`;
    chatWindow.prepend(newMessage); // Añadir el mensaje al principio del contenedor
    chatWindow.scrollTop = 0; // Ajustar el scroll para que esté al principio
  } else {
    const chatWindowPrive = document.getElementById("chat-private");
    const messageContent = `(privado) ${msg.message}`;
    const newMessage = document.createElement("div");
    newMessage.textContent = `${msg.username}: ${messageContent}`;
    chatWindowPrive.prepend(newMessage); // Añadir el mensaje al principio del contenedor
    chatWindowPrive.scrollTop = 0; // Ajustar el scroll para que esté al principio
  }
});
/*
socket.on("chat message", (msg) => {
