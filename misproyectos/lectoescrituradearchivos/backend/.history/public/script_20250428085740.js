document.getElementById('apiForm').addEventListener('submit', async (event) => {
    ...
    try {
        ...
        // Envía la solicitud al endpoint de nuestro servidor (/api/gemini)
        ...
        const valueRespuesta = result.candidates[0].content.parts[0].text;

        // Muestra la respuesta de la API (que nos envió nuestro servidor) en el frontend
        output.innerHTML = `<strong>Respuesta de la API:</strong><pre>${valueRespuesta, null, 2)}</pre>`;
