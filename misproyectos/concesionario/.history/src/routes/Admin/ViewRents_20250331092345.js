...
function ViewRents() {
  ...
  const eliminarUser = async (id) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        await fetchRentData(); // Actualizar la lista después de eliminar
      } else {
        const errorData = await response.json();
        setError(errorData.error);
  ...
  // Funciones vacías para implementar
      ...
      <main className="content">
        ...
        <div id="usuariosContainer">
          <table className="user-table-horizontal">
            ...
            <tbody>
              {rentData.map((rent) => (
                <tr key={rent.id_usuario} className="cuerpo-tabla">
                  ...
                  <td>
                    <button onClick={() => openModal(rent.id_usuario)}>Acción</button>
                  </td>
                </tr>
              ))}
            </tbody>
