...
io.on('connection', (socket) => {
    /*socket.on('updateUser', () => {
        ...
        // REVISAR
        // Añadido: actualizar el estado online al conectarse
        db.run('UPDATE users SET online = 0 WHERE identifier = ?', [identifier], (err) => {
            if (err) {
                console.log('Error updating status on connect');
            }
            updateOnlineStatus(); // Añadido
        });
    });*/
        // REVISAR
    ...
    socket.on('check-ip', () => {
        ...
        db.get('SELECT * FROM users WHERE identifier = ?', [identifier], (err, row) => {
            ...
            if (row) {
                ...
                socket.emit('ip-checked', { registered: true, username: row.username });
                userSockets.set(row.username, socket.id);
            } else {
                // Si el identificador no está registrado
                socket.emit('ip-checked', { registered: false });
            }
        });
        db.run('UPDATE users SET online = 0 WHERE identifier = ?', [identifier], (err) => {
            if (err) {
...
//        db.run('INSERT INTO messages (username, message, timestamp) VALUES (?, ?, ?)', [msg.username, msg.message, new Date().toISOString()]);
    ...
    socket.on('disconnect', () => {
        db.run('UPDATE users SET online = 1 WHERE identifier = ?', [socket.ipAddress], (err) => {
            if (err) {
...
app.get('/online-users', (req, res) => {
    db.all('SELECT username FROM users WHERE online = 0', [], (err, rows) => {
        ...
        res.json(rows);
        io.emit('online-users', rows); // Emitir evento en tiempo real
    });
});
