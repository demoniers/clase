...
function RentalPanel() {
  ...
  const [cars, Rental, setCars] = useState([]); // Estado para los coches
  ...
  useEffect(() => {
    ...
    const fetchRents = async () => {
      try {
        const response = await fetch('http://10.0.0.124:5000/api/alquileres'); // Petición a la API de coches
        if (response.ok) {
          const rentalData = await response.json();
          setRental(rentalData); // Guardar los coches en el estado
        } else {
          console.error('Error al obtener coches de la API.');
        }
      } catch (err) {
          ...
          {cars.length === 0 ? (
            ...
            cars.map((car) => (
              <div className="profile-info" key={car.id}>
                ...
                <p>Precio por día: <strong>{car.precio_dia}€</strong></p>
                <p>Estado: <strong>{car.estado}</strong></p>
                <button
      ...
      {isModalOpen && selectedCar && (
        <div className="modal">
          <div className="modal-content">
            ...
            <p>Precio por día: <strong>{selectedCar.precio_dia}€</strong></p>
            <button onClick={closeModal} className="modal-close-btn">Confirmar Reserva</button>
