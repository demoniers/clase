...
function VerAlquiler() {
  ...
  const rentFinish = async (id) => {
    const token = localStorage.getItem('token');
    try {
      const response = await fetch('http://10.0.0.124:5000/api/rentFinish', {
        method: 'POST',
        headers: {
          Authorization: token,
          alquiler: id,
      ...
      if (response.ok) {
        alert('El alquiler a sido finalizado.');
        closeModal();
      } else {
        alert('No se pudo final el alquiler, ocurrio un error.');
      }
    } catch (error) {
      console.error('Error al obtener los alquileres:', error);
      setError('Error al cargar los alquileres.');
      ...
      {isModalOpen && (
        <div className="modal">
          <div className="modal-content">
            ...
            <p>Acciones del alquiler con ID: {selectedRentId}</p>
            <button onClick={ () => rentFinish(selectedRentId)} className="modal-close-btn">Finalizar Alquiler</button>
