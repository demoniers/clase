...
// Verificar MAC y registrar usuario si es necesario
app.post('/verify-identifier', (req, res) => {
    const { username } = req.body;
    const macAddress = getmac.default();

    db.get('SELECT * FROM users WHERE identifier = ?', [macAddress], (err, row) => {
        if (err) {
        ...
        if (row) {
            ...
            db.run('INSERT INTO users (username, identifier, online) VALUES (?, ?, ?)', [username, macAddress, 1], function(err) {
                if (err) {
