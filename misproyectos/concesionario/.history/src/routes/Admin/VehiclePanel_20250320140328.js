...
function VehiclePanel() {
  ...
  const [backgroundPreview, setBackgroundPreview] = useState(''); // Almacena la URL de la vista previa
  ...
  if (file) {
    setImagen(file); // Guarda la imagen seleccionada
    const previewURL = URL.createObjectURL(file); // Crea la URL temporal
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://10.0.0.124:5000/api/cochesAniade', {
        method: 'POST',
        body: formDataToSend,
      });
  ...
  return (
    ...
    style={{backgroundImage: backgroundPreview ? `url(${backgroundPreview})` : 'none', backgroundSize: 'cover', backgroundPosition: 'center',
    }}>
