...
app.post('/api/viewalquileres', (req, res) => {
  db.all(`
    SELECT a.id_coche, c.*, u.id AS id_usuario, u.*, a.id AS id_alquiler, a.fecha_inicio, a.fecha_fin, a.precio_total
    FROM alquileres a
    JOIN coches c ON a.id_coche = c.id
...
app.post('/api/rentFinish', (req, res) => {
  ...
  try {
    ...
    if (level > 0) {
      db.all("SELECT id_coche FROM alquileres WHERE id = ?", [id], function (err, rows) {
        if (err) {
        ...
        if (rows.length === 0) {
          res.status(404).json({ error: "No se encontró ningún alquiler con el ID proporcionado." });
          return;
        }

        const id_coche = rows[0].id_coche; // Extrae el valor de id_coche

        // Actualizar el estado de la tabla alquileres
        db.run('UPDATE alquileres SET estado = 0 WHERE id = ?', [id], function (err) {
          if (err) {
            res.status(500).json({ error: err.message });
          } else {
            // Aquí puedes realizar más acciones, como actualizar otra tabla
            db.run('UPDATE coches SET disponible = 1 WHERE id = ?', [id_coche], function (err) {
              if (err) {
                res.status(500).json({ error: err.message });
              } else {
                res.status(200).json({ message: 'Registro actualizado exitosamente.', id: id, id_coche: id_coche });
              }
            });
          }
        });
