...
//    const apiKey = process.env.API_KEY; // Usa una variable de entorno para la clave API
  ...
  if (!inputData || typeof inputData !== 'string') {
      return res.status(400).json({ error: 'Datos inválidos. Debes enviar datos en formato "NombreDelCampo:ValorNumerico,...".' });
  }

  let prompt = `Genera un gráfico de ${inputData}. `;
  prompt += "Usa el formato 'NombreDelCampo:ValorNumerico,...'. Responde únicamente con los datos para hacer el gráfico en este formato.";
  ...
  try {
      ...
      if (!Array.isArray(data) || data.length === 0) {
          return res.status(400).json({ error: 'Datos inválidos. Verifica el formato de la respuesta de la API.' });
      }
      ...
      res.send(svg.node().outerHTML, textData);
      */
      res.header("Content-Type", "application/json");

      // Envía el objeto JSON con el SVG y el texto
      res.json({
          svg: svg.node().outerHTML, // SVG generado
          text: textData // Texto adicional
      });
      ...
      if (error.response) {
          ...
          res.status(500).json({ error: 'Error interno al conectar con la API de Gemini.' });
      }
  }
});
